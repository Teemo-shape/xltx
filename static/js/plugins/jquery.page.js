...
(function($){
	...
	function Page(container,fn,args,style){
		...
		var height=this.height=43;//parseInt(this.obj.css('height'));
		...
		var style=this.style=$.extend({
			...
			"prevNextWidth":88,//上页下页的宽度
			"pagecountWidth":74,//共计多少页的宽度
			"trunWidth":216//跳转模块宽度
		},style||{});
		
		if(!style.pageWidth)style.pageWidth=52;/*(function(){
	...
	Page.prototype.fillHtml=function(){
		...
		if(args.showPrev){
			if(args.current > 1){
				obj.append('<a href="#/page'+(args.current-1)+'" class="prevPage">&lt;上一页</a>');
			}else{
				obj.remove('.prevPage');
		...
		if(args.showNext){
			if(args.current== args.pageCount||args.pageCount==1){
				...
				obj.append('<a href="#/page'+(args.current+1)+'" class="nextPage">下一页&gt;</a>');
			}
		}
	...
	Page.prototype.setStyle=function(){
		...
		this.obj.find('a').css({'border':'1px solid '+s.borderColor});
		...
		this.obj.find('span.disabled').css({'height':s.height+'px','color':s.disableColor,'background':s.disableBackColor,'border':'1px solid '+s.borderColor,'width':s.prevNextWidth+'px'});
